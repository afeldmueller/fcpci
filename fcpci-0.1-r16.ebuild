...
src_unpack() {
	...
	if kernel_is ge 2 6 39 ; then
		...
		if kernel_is ge 4 1 0 ; then
			epatch "${FILESDIR}"/fcpci-kernel-4.1.0.patch
		...
		if kernel_is ge 4 12 0 ; then
			epatch "${FILESDIR}"/fcpci-kernel-4.12.0.patch
		fi
	...
	epatch "${FILESDIR}"/fcpci-gcc-4.9.patch
